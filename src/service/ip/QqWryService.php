...
class QqWryService extends Service
{
    ...
    {
        $file = app()->getRootPath() . 'runtime/dtapp/ip/';
        is_dir($file) or mkdir($file, 0777, true);
        $this->fp = 0;
        if (($this->fp = fopen($file . '/qqwry.dat', 'rb')) !== false) {
    ...
    private function getNameAdCodeLatLng($province_name, $city_name, $district_name)
    {
        ...
        if (!empty($province_name)) {
            try {
                $json_province = json_decode(file_get_contents(app()->getRootPath()  . 'runtime/dtapp/ip//province.json'), true);
        ...
        if (!empty($city_name)) {
            try {
                $json_city = json_decode(file_get_contents(app()->getRootPath()  . 'runtime/dtapp/ip//city.json'), true);
        ...
        if (!empty($district_name)) {
            try {
                $json_district = json_decode(file_get_contents(app()->getRootPath()  . 'runtime/dtapp/ip//district.json'), true);
